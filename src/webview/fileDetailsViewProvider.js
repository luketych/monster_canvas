...
class FileDetailsViewProvider {
  ...
  _getHtmlForWebview(webview) {
    ...
    // Get styles content
    const styleContent = fs.readFileSync(path.join(this.context.extensionPath, 'src', 'webview', 'styles', 'fileDetailsStyles.css'), 'utf8');
    html = html.replace('{{styles}}', `<style>${styleContent}</style>`);

    // Get script content
    const scriptContent = fs.readFileSync(path.join(this.context.extensionPath, 'src', 'webview', 'js', 'fileDetails.js'), 'utf8');
